...
| `apply_sync_recommended` | Pending rows can be replayed by `graph.apply_sync()` |
| `maintenance_recommended` | Overlay, read-only, vacuum, or rebuild state should be compacted |

Use this from `pg_cron`, an external scheduler, or monitoring checks when you
need a single query that decides whether to call `graph.apply_sync()` or
`graph.maintenance(concurrently := true)`. It does not schedule work by itself.
...
Watch edge_buffer_used, pending_sync_rows, disabled_trigger_count, and read_only.
```
