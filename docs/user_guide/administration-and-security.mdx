...
`search_path` to `pg_catalog, public`. This prevents objects in a
caller-controlled schema from changing how those privileged functions resolve
built-in names while retaining compatibility with legacy registrations that
refer to tables in `public` without a schema qualification. Applications should
still schema-qualify their own database objects and grant only the graph
functions each role requires.
...
|---|---|---|---|
| `53200` | `PG001` | Memory limit exceeded | Raise `graph.memory_limit_mb`, reduce registered graph size, or change OOM policy |
