...
| Release history |
|---|
| [v1.2.1 candidate](#v1-2-1-pggraph-stability-candidate) |
| [v1.2.0](#v1-2-0-pggraph-open-vocabulary-relationship-types) |
| [v1.1.0](#v1-1-0-pggraph-caller-scoped-rls-and-safe-replacement) |
| [v1.0.0](#v1-0-0-pggraph-production-release) |
...
## v1.2.1: pgGraph Stability Candidate

pgGraph 1.2.1 is a stability candidate, not yet tagged or published. It fixes
sync replay, registration, artifact isolation, and transactional graph
publication. PostgreSQL 14 through 18 remain the supported target matrix.

### Correctness And Security

- `csr_readonly` replay accepts valid new relationship labels, including the
  first label after an empty build. Rolled-back mutations and fixed-snapshot
  replay state no longer leak into later backend queries.
- Dotted JSONB property registration preserves INSERT, UPDATE, and mapped GQL
  writes. Invalid filter columns and incompatible types report input errors
  instead of internal-bug diagnostics.
- Literal dotted source-column names, including a `label_column` named
  `rel.type`, remain SQL identifiers in generated sync triggers.
- Mandatory multi-pattern GQL joins apply a supported primary-key equality on
  ...
  enforced; broader property scans retain their documented limits.
- Ordinary node-only GQL reads evaluate property predicates before retaining
  ...
  [documented candidate limits](./known-issues#gql-candidate-limits-depend-on-the-query-shape).
- GQL `DETACH DELETE` resolves transaction-created nodes and removes their
  ...
  GQL reads, relationship creation and rollback.
- Mapped GQL relationship `DELETE` resolves transaction-created endpoints and
  preserves parallel relationships, reverse orientations and rollback.
- Artifacts are isolated by database and validated against registered catalog
  identities. Default graphs cannot adopt another database's files.
- Graph generations publish through PostgreSQL transactions. Transaction and
  ...
  fence before the surrounding query transaction ends.
- Build and reload reject ambiguous or mismatched endpoint identities,
...
[artifact lifecycle](/user_guide/build-and-persistence#reset).

### Upgrade, Rebuild, And Rollback

Back up PostgreSQL before installing the matching 1.2.1 candidate package.
...
The rebuild is required for database-scoped roots, catalog provenance, and
...
exact-result, [1M](../release/evidence/engine/2026-09-11-p5-selective-rls-1m/README.md)
and [10M selective-RLS evidence](../release/evidence/engine/2026-09-12-p5-selective-rls-10m/README.md)
cover their stated profiles and resource envelopes. Candidate release gates
remain in progress; these notes do not certify published packages or universal
performance guarantees.

## v1.2.0: pgGraph Open-Vocabulary Relationship Types
...
- Relationship types now use a checked logical `EdgeTypeId(u32)`. A graph may
  contain up to 1,000,000 distinct types, with a 1,024-byte UTF-8 limit per
  label and a 256 MiB cumulative dictionary limit.
- New v7 bases and mutable segments select one-, two-, or four-byte type
  storage, so graphs with small vocabularies retain byte-sized edge type IDs.
- Trigger-backed committed `graph.apply_sync()` interns valid unseen labels
  and publishes the updated dictionary without requiring a full rebuild.
- `traverse`, shortest paths, explicit GQL/Cypher relationship types, and
  eligible `WHERE r.relationship_name = $x` predicates use exact compact type
  filters. Absent types return no match; ambiguous endpoint mappings fail
  closed.
- `graph.edge_types()` pages the complete effective dictionary in stable ID
...
- Auto-discovery recognizes supported binary relationship tables as one edge
  ...
  columns such as `relationship_name`.
- Composite foreign keys, repeated endpoint columns, alternate unique-key
...
- The new `graph.reset(boolean)` overload provides a direct recovery path for
  stale registrations. `graph.reset(true)` clears the selected graph's
  projection, artifacts, tables, edges, and filter registrations.
- The zero-argument `graph.reset()` call remains projection-only and preserves
  registrations for a rebuild.
- Neither form modifies PostgreSQL source tables or other named graphs. After
  `graph.reset(true)`, reapply the selected graph's registrations before
  calling `graph.build()`.
- See [Stale Registration Produces PG000](/user_guide/troubleshooting#stale-registration-produces-pg000)
  for the recovery sequence.

### Upgrade, Artifacts, And Rollback

- The `1.1.0 -> 1.2.0` extension update is additive: it installs
  `graph.edge_types()` and `graph.reset(boolean)` without replacing existing
  function objects or changing their owners and explicit grants.
- Existing v6 artifacts remain readable without a blanket rebuild. New builds
  publish v7 artifacts. Rebuild when the base artifact itself must represent
  more than 254 relationship types.
- In-place downgrade is not supported. A 1.1 binary cannot read 1.2 v7
...
- Retained exact-commit evidence covers 65 Criterion cases, eight PostgreSQL
  latency cases, and Linux resource runs at 1, 4, and 8 backends.
- The 65,536-label fixture used 54.7 bytes per directed edge against a
  64-byte budget. Its measured 8-to-1 total and baseline-subtracted query PSS
  ratios were 5.86x and 6.36x against 9.0x limits.
- These measurements verify the release budgets in the recorded environment;
  they are not universal latency or memory guarantees.

## v1.1.0: pgGraph Caller-Scoped RLS And Safe Replacement

pgGraph 1.1 makes PostgreSQL source-table RLS authoritative for graph topology,
...
- See [Versioning And Compatibility](./user_guide/versioning-and-compatibility)
...
The release was validated with retained RC/full-matrix evidence, clean PostgreSQL
14–18 source-package installation, the signed alpha migration fixture,
backup/restore and crash recovery, concurrency and transaction gates, resource
measurements, sanitizers, fuzz/Miri checks, and packaged quickstart and
playground examples. The source bundle contains one commit-bound PGXN ZIP,
SPDX 2.3 SBOM, provenance, manifest, and complete SHA-256 set. Publication must
consumed those exact prepared bytes after signed-tag verification without
