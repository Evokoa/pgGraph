...
<Callout type="info">
These instructions target pgGraph 1.2.1 on PostgreSQL 14 through 18.
Upgrading requires updating the extension and rebuilding each graph as
described in [release notes](/release-notes).
</Callout>
...
The formula uses a signed release bundle. For this release, verify that the
installed extension reports version `1.2.1`.
...
and announces availability only after the installed extension reports 1.2.1.
...
For PGXN installation, use the exact checksummed source ZIP from the signed
1.2.1 release bundle. Because pgGraph is a Rust/pgrx extension, source
installation requires Rust and `cargo-pgrx` before running `pgxn install`.
...
```bash
docker pull ghcr.io/evokoa/pggraph:1.2.1
```

| Tag | Description |
|---|---|
| `1.2.1` | Pinned reproducible tag (PostgreSQL 17), always resolves to the same image digest |
| `latest` | Moving tag that points to the most recent stable PostgreSQL 17 image |
| `pg14-1.2.1` through `pg18-1.2.1` | Versioned image for a specific PostgreSQL major |
