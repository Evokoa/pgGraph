...
read-mostly engine. `mutable_overlay` is an opt-in build mode for the upcoming
transaction-local write surface; selecting it requires
`graph.mutable_enabled = on`.
