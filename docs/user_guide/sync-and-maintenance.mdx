...
Durable filter values use projection segment format v3. Upgrading from a
release that wrote segment format v2 requires a fresh `graph.build()` before
the persisted mutable graph is served. The rebuild reads PostgreSQL source
tables, which remain authoritative; it does not require rewriting application
rows or graph registrations.
