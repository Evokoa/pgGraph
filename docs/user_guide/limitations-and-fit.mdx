...
| Fast backend startup | Persisted forward graph arrays and resolution bytes can be mmap-loaded |
...
| mmap sharing | Forward graph arrays and resolution bytes are mmap-backed; reverse CSR and bincode metadata are per-backend heap |
| Query catch-up | Default query behavior does not apply pending sync rows; `graph.query_freshness = 'apply_pending_sync'` currently opts single-start `graph.traverse()` into bounded catch-up |
