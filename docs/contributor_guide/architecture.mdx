...
| Build jobs | `graph._build_jobs` and `graph._maintenance_jobs` are keyed by `graph_id`; workers restore database, user, and graph context before running. |
...
| Discovery | `graph.auto_discover()` and `graph.auto_discover_tables()` can target a named graph or the selected/default graph. |
