...
| GQL | `graph.gql()` supports the documented read subset, including node-only `MATCH`, top-level single-relationship `OPTIONAL MATCH`, projection-stage `WITH`, `RETURN` aggregates, single-node mapped `CREATE`, single-node mapped property `SET`, and single-edge mapped `DELETE`; path functions, SQL/PGQ, and broader writes are future work |
