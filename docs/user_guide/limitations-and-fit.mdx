...
| GQL | `graph.gql()` supports the documented read subset, including node-only `MATCH`, single-node mapped `CREATE`, and single-node mapped property `SET`; aggregates, path functions, multi-stage queries, and broader writes are future work |
