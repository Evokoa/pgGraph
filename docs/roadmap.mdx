...
inside PostgreSQL, without turning pgGraph into a separate graph database or a
...
| Persistence | Atomic `.pggraph` artifacts under PostgreSQL data storage. Forward graph arrays and resolution are mmap-backed on load; reverse CSR and bincode metadata are backend-local heap structures. |
...
| Packaging | Tighten source install, package validation, fresh install smoke tests, and PostgreSQL 13-18 release evidence. |
...
| Benchmarks | More end-to-end SQL benchmarks that separate PostgreSQL execution, graph traversal, hydration, and client overhead. |
...
| P2 | SQL ergonomics and documented semantics | More structured-filter operators, case-insensitive search index guidance, date/timestamp filter cleanup. |
| P3 | Internals and maintainability | Module cleanup, build/vacuum pipeline extraction, filter-op reshaping, test locality. |
...
| Online mutable graph segments | Not part of the current query path. | Investigate append or delta structures that reduce rebuild pressure without weakening traversal correctness. |
...
| A Cypher, Gremlin, SPARQL, or GQL implementation | The public interface is SQL functions in the `graph` schema. |
