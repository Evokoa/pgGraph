...
after segment publication succeeds. SQL entrypoints, scheduled maintenance
wiring, and rollback-heavy verification are added in the later ingestion slice.
...
manifests; the engine keeps only the base manifest generation and sync watermark
as status metadata.
