...
| Forward `EdgeStore` mapped mode | Validated ranges with an `Arc` to the immutable snapshot | The store owns its mapping independently of engine field/drop order |
...
<PipelineDiagram>
  ...
  <PipelineStep title="validate header, version, CRC, sections, CSR, PK offsets, bincode bounds">
    ...
    - heap edge metadata from bincode
    - heap reverse_edge_store from forward CSR and relationship IDs
  </PipelineStep>
