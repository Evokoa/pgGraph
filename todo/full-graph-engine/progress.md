...
| 1A. Security and identity | In progress | Coordinate-only node visibility, mapped single-pattern relationship-row visibility, base-CSR relationship multiplicity, table-qualified filter identity, persisted relationship identity sidecars, base-CSR query/path ID propagation, and base relationship hydration are enforced on PostgreSQL 17; join/wildcard relationship RLS, overlays, sync, compaction, writes, and savepoints remain. |
...
- Preserved parallel source relationship rows through base CSR construction,
  ...
  separate edge-row IDs. Mapped single-pattern relationship rows are checked
  under caller RLS before coordinate-only output. Join/wildcard relationship
  RLS, mutable overlays, sync, compaction, and writes remain open.
