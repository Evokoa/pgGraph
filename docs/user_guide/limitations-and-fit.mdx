...
| Validated persisted startup | Forward graph arrays and resolution bytes load into one compact immutable mapping per backend |
...
| mmap sharing | Forward graph arrays, schema-direction flags, and resolution bytes are read-only mmap-backed derived artifact sections; reverse CSR and bincode metadata are per-backend heap |
