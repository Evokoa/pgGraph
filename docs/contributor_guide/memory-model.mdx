...
created. Format v5 keeps both CSR directions, filter values and dictionaries,
...
<Callout type="info">
The base graph is copied once into each loading backend's anonymous
...
view that mapping. Each backend still has a private virtual mapping; resident
pages are accounted as private mapped/page-cache-backed bytes rather than
described as shared extension memory.
</Callout>
...
<StructDiagram title="EdgeStore" fields={[
  ...
  { name: 'relationship_ids', type: 'Vec<u32> or [u32]', desc: 'parallel source identity IDs' }
]} />
...
When the engine is loaded from a `.pggraph` file, the anonymous artifact
snapshot and its resident page-cache-backed bytes are counted as
backend-private mapped memory. Filter and relationship-identity deltas, the
registry, and other overlays are additional backend-private heap allocations.
The public shared-memory profile columns remain for 1.x compatibility and
