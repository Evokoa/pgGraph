...
The current artifact format is v5. Its 448-byte header describes 23 aligned
...
and relationship identities remain in the immutable snapshot. Post-load filter
changes use sparse deltas, and newly observed relationship identities use an
owned suffix overlay.
...
<FlowDiagram
  ...
  steps={[
    ...
    },
    {
      ...
      description: "Stream sorted temp edges and schema-direction flags into forward and inbound CSR stores."
    },
    {
...
When persistence is enabled, pgGraph writes the owned build result, drops that
owned graph, and then reloads the persisted artifact through mmap validation.
This avoids holding both the owned replacement and the loaded mmap projection
at the same time.
...
5. If the artifact exists, `load_graph_file()` validates the v5 header, both
   CRCs, all 23 descriptors, and section contents before exposing immutable
   sections read-only.
...
| `.pggraph` format version changed | Version 4, earlier, and future artifacts fail closed and must be rebuilt |
