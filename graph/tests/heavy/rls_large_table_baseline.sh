...
case "$RUN_PROFILE" in
  full|compact) ;;
  *) echo "RUN_PROFILE must be full or compact"; exit 2 ;;
esac
...
CREATE TABLE public.rls_bench_samples (
    ...
    spi_calls text NOT NULL DEFAULT 'NA',
    keys_scanned text NOT NULL DEFAULT 'NA',
    bytes_scanned text NOT NULL DEFAULT 'NA'
);
...
BEGIN
    ...
    IF sample_number > 0 THEN
        INSERT INTO public.rls_bench_samples (
            case_name, key_shape, rls_scope, policy_shape, query_kind,
            sample, total_ms, result_rows, visibility_us,
            hidden_nodes, hidden_relationships
        ) VALUES (
            case_name, key_shape, rls_scope, policy_shape, query_kind,
            sample_number,
...
END
$benchmark$;
GRANT EXECUTE ON FUNCTION public.rls_bench_measure(text,text,text,text,text,text,bigint,integer) TO :role_name;
SQL
...
SQL
    for ((iteration = 1; iteration <= WARMUPS; iteration++)); do
      printf "SELECT public.rls_bench_measure(:'case_name', :'key_shape', :'scope', :'policy', :'query_kind', :'query_sql', :expected_rows, 0);\n"
    done
    for ((iteration = 1; iteration <= SAMPLES; iteration++)); do
      printf "SELECT public.rls_bench_measure(:'case_name', :'key_shape', :'scope', :'policy', :'query_kind', :'query_sql', :expected_rows, %d);\n" "$iteration"
    done
