...
| Querying | Bounded traversal, shortest paths, weighted shortest paths, path reconstruction, source search, workflow helpers, connected components, and a GQL-compatible subset through `graph.gql()`. |
...
| Artifact tooling | More direct inspection and compatibility checks for persisted graph files, plus a future pgGraph-owned binary encoding for variable metadata sections so artifact compatibility is not tied to serializer crate behavior. |
| Query ergonomics | More examples around strict traversal specs, typed filters, workflow helpers, pagination, and relationship registration patterns. |
...
|---|---|---|
| P0 | Operational visibility and resource predictability | No active public rows after instance-level memory sizing visibility was added. |
| P1 | Query performance follow-ups | Dirty mutable-projection hot-path optimization is tracked in Known Issues and should be benchmark-driven. |
| P2 | SQL ergonomics and documented semantics | Cypher preview and SQL/PGQ public-surface boundaries are tracked in Known Issues; broader language support remains roadmap-owned. |
| P3 | Internals and maintainability | Mega-file splits and additional weak-path tests are tracked in Known Issues. |
...
| Broader GQL, openCypher, and SQL/PGQ frontends | A GQL-compatible subset, including node-only `MATCH`, single-relationship reads, multi-pattern joins over concrete labels/types with node-property `WHERE` predicates, bounded variable-length join path variables, relationship variable returns for fixed single-hop join patterns, variable-length relationship variables as path values, path variable returns, path functions, ordering, and projected-row `RETURN DISTINCT`, top-level single-relationship `OPTIONAL MATCH`, fixed path variables with optional named nodes and concrete labels/types plus compatible path-node property predicates, bounded wildcard variable-length path variables including unnamed multi-segment variable-length segments, relationship type alternation, and named endpoint variables plus relationship variables as path values on single bounded segments, projection-stage `WITH`, bounded `DISTINCT`, `RETURN` aggregates, JSONB list/map property paths, bounded relationship path values and path functions, single-node mapped `CREATE`, single-node mapped property `SET`, mapped property `REMOVE`, single-edge mapped `DELETE` including wildcard relationship patterns that resolve to exactly one static-label registered edge-row mapping, single-node `DETACH DELETE` over registered edge-row relationships, and single-node mapped `MERGE` are public through `graph.gql()`. `graph.cypher()` exposes the overlapping openCypher-shaped subset through the same planner with a separate compatibility matrix and no full openCypher compatibility claim; an internal typed SQL/PGQ adapter seam exists, but SQL/PGQ is not a current public API. | Extend GQL coverage to relationship source-row hydration, relationship property projection, optional path-variable projection semantics, broader wildcard property predicates, and expose SQL/PGQ only after PostgreSQL provides stable graph-pattern hooks and the compatibility rows are green. |
| Online mutable graph overlays | The `mutable_overlay` build mode, transaction-delta lifecycle, overlay-aware read paths, PostgreSQL-first GQL node `CREATE`, PostgreSQL-first GQL property `SET`, PostgreSQL-first GQL property `REMOVE`, PostgreSQL-first GQL edge-row `DELETE`, PostgreSQL-first GQL node `DETACH DELETE`, and PostgreSQL-first GQL node `MERGE` are present. | Continue routing narrow GQL writes through PostgreSQL-first DML and transaction-local deltas while keeping `csr_readonly` as the fast read-mostly mode. |
| Broader GQL writes | Current GQL writes are intentionally narrow: single-node mapped `CREATE`, mapped property `SET` and `REMOVE`, single-edge mapped `DELETE`, single-node `DETACH DELETE`, and single-node mapped `MERGE`. Writes are rejected on `csr_readonly` projections and continue to use PostgreSQL source rows as the authority. | Consider relationship creation, multi-row writes, dynamic labels, and GQL-driven PostgreSQL schema creation only after the narrow PostgreSQL-first write boundary remains stable under concurrency, rollback, ACL, and SQLSTATE tests. |
| Non-CSR mutable projection | Not current behavior. Current mutable behavior still uses immutable base CSR plus overlays, so maintenance/vacuum is needed to compact committed topology changes. | Investigate an opt-in projection mode built around durable manifest generations, immutable delta CSR segments, layered neighbor reads, compaction, and generation-aware garbage collection. This is the concrete LSM-like adjacency-runs path for reducing routine full rebuild pressure while preserving bounded traversal, PostgreSQL source-of-truth semantics, crash safety, and predictable memory behavior. |
