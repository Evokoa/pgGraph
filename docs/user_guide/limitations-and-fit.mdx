...
| GQL and openCypher | `graph.gql()` supports the documented read subset, including node-only `MATCH`, top-level single-relationship `OPTIONAL MATCH`, fixed single-hop multi-pattern joins over concrete labels/types with node-property `WHERE` predicates, ordering, and projected-row `RETURN DISTINCT`, fixed path variables with optional named nodes and concrete labels/types plus compatible path-node property predicates, bounded single-segment wildcard variable-length path variables, projection-stage `WITH`, bounded `DISTINCT`, `RETURN` aggregates, JSONB list/map property paths, bounded relationship path values and path functions, single-node mapped `CREATE`, single-node mapped property `SET`, mapped property `REMOVE`, single-edge mapped `DELETE` including wildcard relationship patterns that resolve to exactly one static-label registered edge-row mapping, single-node `DETACH DELETE` over registered edge-row relationships, and single-node mapped `MERGE`; `graph.cypher()` accepts the overlapping openCypher-shaped subset through the same planner and explicitly does not claim full openCypher compatibility; multi-pattern relationship variables/path variables and broader writes are future work |
