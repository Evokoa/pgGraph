...
`v0.1.5` hardens the GQL mutable-overlay path, fixes several edge-case
semantics found during review, and makes the Docker-backed playground release
gate validate the current checkout instead of a stale container image. It keeps
the `v0.1.4` installation and SQL function surface compatible.
