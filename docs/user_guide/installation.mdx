...
The pgGraph 1.0.0 source tree is release-ready but unpublished. Build from the
reviewed source commit until signed PGXN and GHCR artifacts and the matching
Homebrew formula are announced.
</Callout>
...
## Homebrew Install After Publication

After 1.0.0 publication, the
[Evokoa Homebrew tap](https://github.com/Evokoa/homebrew-tap) provides the
PostgreSQL 17 convenience formula. Before installing, verify that its version
and checksum match the signed release bundle; until then, the tap may still
...
release bundle. Before publication, build the repository's Compose
image locally.

The Homebrew formula is a convenience channel for PostgreSQL 17. After
publication, the complete 1.0 package matrix is the checksummed PGXN/source
archive and attested GHCR images for PostgreSQL 14 through 18.
...
and announces availability only after the installed extension reports 1.0.0.
...
## PGXN Source Install After Publication

After publication, PGXN serves the exact checksummed source ZIP from the signed
1.0.0 release bundle. Because pgGraph is a Rust/pgrx extension, source
...
| Path | Use when |
|---|---|
| Pull the pre-built image from GHCR after publication | You want the fastest start with no build step |
...
After 1.0.0 publication, attested images are available on GitHub Container
Registry for PostgreSQL 14 through 18 and support **linux/amd64** and
**linux/arm64**. Until then, use the local image-build instructions below; the
candidate tags shown here are post-publication commands, not current
availability claims.
...
| Tag | Description |
|---|---|
| `1.0.0` | Pinned reproducible tag (PostgreSQL 17) — always resolves to the same image digest |
| `latest` | Moving tag that points to the most recent stable PostgreSQL 17 image |
| `pg14-1.0.0` through `pg18-1.0.0` | Versioned image for a specific PostgreSQL major |
...
`$PGDATA/<graph.data_dir>/<graph_id>/`. The `graph_id` is the stable directory
