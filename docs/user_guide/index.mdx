...
including a documented GQL-compatible subset through `graph.gql()`. SQL/PGQ
adapters and broader graph-language coverage remain future work.
</Callout>
...
| GQL | A documented GQL-compatible graph-pattern subset, single-node mapped `CREATE`, and single-node mapped property `SET`, returned as JSONB rows |
