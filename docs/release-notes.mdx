...
| Release history |
|---|
| [v1.0.0](#v1-0-0-production-contract) |
...
`v1.0.0` freezes pgGraph's first production contract for PostgreSQL 14 through
18. The source tree currently contains the release candidate; it is not yet
tagged or published. PostgreSQL source tables remain authoritative, while graph
artifacts and mutable projection generations remain validated, rebuildable
derived state.
...
### Candidate Validation And Distribution

Before release, the candidate must pass retained RC/full-matrix evidence, clean
PostgreSQL 14–18 source-package installation, the signed alpha migration
fixture, backup/restore and crash recovery, concurrency and transaction gates,
resource measurements, sanitizers, fuzz/Miri checks, and packaged quickstart
and playground examples. The prepared source bundle will contain one
commit-bound PGXN ZIP, SPDX 2.3 SBOM, provenance, manifest, and complete SHA-256
set. Publication consumes those exact prepared bytes after signed-tag
verification; it does not rebuild the archive.
