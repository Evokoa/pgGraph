...
Build and maintenance operations enforce the relationship type limits in
[Limitations And Fit](./limitations-and-fit); failures retain the last valid
generation.
...
where PostgreSQL says RLS applies to the effective SQL execution role, pgGraph scans
...
Post-1.1 direct probes for `get_node()` and true depth-zero traversal use
ordinary primary-key equality for built-in key types whose text representation
is independent of session formatting settings. If a topology shape can consume
an RLS-active node or relationship mapping whose projected identity uses a
date/time, array, domain, extension, or custom type, that shape fails closed
with `0A000` instead of matching text produced under possibly different session
settings. Direct `get_node()` probes validate the requested node mapping;
expanding and whole-graph shapes validate every mapping they can consume.
Positive-depth and whole-graph shapes continue to use eager scans for supported
stable key types while the resumable traversal work proceeds.
...
An RLS policy must not call a pgGraph topology function that resolves the same
policy recursively. pgGraph rejects that shape with SQLSTATE `55000` and
diagnostic `PG024`; keep policy predicates in ordinary PostgreSQL relations or
session settings and invoke topology outside the policy.
...
SQL used for hydration and visibility checks therefore sees the effective SQL
role for policy evaluation and source SQL. pgGraph does not switch Rust user IDs.
If an application deliberately calls pgGraph from its own `SECURITY DEFINER`
wrapper, PostgreSQL changes the effective `current_user` to that wrapper's
owner. Source-table policy queries observe that effective role, just like a
direct `SELECT` executed inside the same wrapper; granting such a wrapper is
therefore a security decision by its owner. pgGraph's explicit source-table ACL
preflight still authorizes the outer application role, so a wrapper does not
delegate source-table `SELECT` privileges that its invoker lacks.
...
RLS-filtered query results. `graph.status()`, `graph.sync_health()`,
build/maintenance job status, `graph.loaded_graphs()`, and
`graph.graph_runtime_status()` require read access
...
and then updates a private PostgreSQL publication row in the same transaction.
The row selects the generation visible to each PostgreSQL snapshot; the
filesystem pointer is advisory. Sync, compaction, repair, and rebuild publishers
compare the transaction-visible generation with the
...
backend while a replacement is prepared. Query startup adopts a newly visible
generation, including for explicitly loaded cold graphs and resident graphs
with automatic initial loading disabled. Backend generation heartbeats and
...
more. PostgreSQL's synchronized visibility horizon also protects transactions
that have not queried the graph yet, including imported snapshots. Collection
under REPEATABLE READ or SERIALIZABLE retains history. A later READ COMMITTED
`graph.projection_gc()` pass can reclaim files and safely applied sync-log rows
after the publishing transaction commits and older readers release their pins.
Sync rows are retained when another graph shares a registered source or the
selected graph has another published artifact root. This conservative policy
prevents one consumer's progress from discarding another consumer's pending
changes; shared-source deployments must account for the retained log volume.
...
published manifest fails checksum validation, pgGraph
...
| `54000` | `PG004` | Edge type limit exceeded | Reduce the relationship type count, label size, or cumulative dictionary payload |
...
| `55000` | `PG024` | Recursive RLS visibility resolution | Remove nested pgGraph topology calls from the source-table RLS policy |
