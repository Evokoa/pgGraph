...
`engine.edge_buffer` as `EdgeMutation` values. Overlay-aware graph algorithms
reduce this buffer into insert and delete overlays for the selected direction.
Traversal, unweighted shortest path, and connected components consume the
shared neighbor-source abstraction. Weighted shortest path rejects a dirty edge
overlay until vacuum or maintenance merges weights into the base CSR.
