...
RUN_RSS="${RUN_RSS:-0}"
RUN_RUNTIME_RESOURCES="${RUN_RUNTIME_RESOURCES:-1}"
RUN_BUILD_MEMORY_STRESS="${RUN_BUILD_MEMORY_STRESS:-0}"
...
RUN_GQL_RELATIONSHIP_RACE="${RUN_GQL_RELATIONSHIP_RACE:-1}"
...
if [[ "$RUN_GQL_RELATIONSHIP_RACE" == "1" ]]; then
  DBNAME="${DB_PREFIX}_gql_relationship_race" PG_VERSION_FEATURE="$PG_VERSION_FEATURE" ./tests/heavy/gql_relationship_race.sh
fi
...
if [[ "$RUN_RUNTIME_RESOURCES" == "1" ]]; then
  DBNAME="${DB_PREFIX}_runtime_resources" PG_VERSION_FEATURE="$PG_VERSION_FEATURE" ./tests/heavy/measure_runtime_resources.sh
fi

if [[ "$RUN_BUILD_MEMORY_STRESS" == "1" ]]; then
  DB_PREFIX="${DB_PREFIX}_memstress" PG_VERSION_FEATURE="$PG_VERSION_FEATURE" ./tests/heavy/build_memory_stress.sh
fi
...
if [[ "$RUN_GQL_ISOLATION_MATRIX" == "1" ]]; then
  DBNAME="${DB_PREFIX}_gql_isolation" PG_VERSION_FEATURE="$PG_VERSION_FEATURE" bash ./tests/heavy/gql_isolation_matrix.sh
fi
