...
resolution bytes are mmap-backed. Forward CSR includes the schema-direction
...
schema-direction flags are shared physically by the OS page cache across
backends; reverse CSR, filter index, registry, and overlay structures remain
per-backend heap.
...
| Type | Validation |
|---|---|
| `MmapNodeArrays::new` | pointer ranges, active byte count, `u32`/`u64` alignment, complete primary-key offsets, terminal byte count, and UTF-8 boundaries |
| `MmapEdgeArrays::new` | pointer ranges, optional weights, `u32` alignment, monotonic CSR offsets, terminal edge count, target bounds, and schema-direction flags |
...
explaining the proof at the call site. Node metadata lookups also check the
requested node index locally and return no value when it is outside the store;
they do not rely on traversal callers to supply a valid index.
