...
| GQL | `graph.gql()` supports the documented read subset, including node-only `MATCH`, and single-node mapped `CREATE`; aggregates, path functions, multi-stage queries, and broader writes are future work |
| SQL/PGQ | Planned adapter work, not a current public API |
| Projection modes | `graph.build()` defaults to `csr_readonly`; `mutable_overlay` can be selected when `graph.mutable_enabled = on` and is required for GQL `CREATE` |
