...
`graph.query_freshness` controls whether opted-in topology reads apply pending
trigger-sync rows before executing. The default is `off`, preserving compatibility
while auto-sync rollout continues.

| Value | Behavior |
|---|---|
| `off` | Reads keep current compatibility behavior and do not apply pending sync rows. |
| `apply_pending_sync` | Opted-in topology reads apply rows up to a captured `_sync_log` high-water mark before reading. |
| `error_on_pending` | Opted-in topology reads fail when pending sync rows exist. |

At this stage, only the single-start `graph.traverse()` path is wired through
the opt-in freshness helper. `graph.apply_sync()` remains the explicit
operator/admin path for all pending sync rows.
