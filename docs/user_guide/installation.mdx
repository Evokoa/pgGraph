...
This extension is built from the `graph/` crate with `pgrx` 0.18.1. The crate
...
| Tag | Description |
|---|---|
| `0.1.7` | Pinned reproducible tag (PostgreSQL 17) — always resolves to the same image digest |
| `latest` | Moving tag that points to the most recent stable PostgreSQL 17 image |
| `pg14-0.1.7` through `pg18-0.1.7` | Versioned image for a specific PostgreSQL major |
