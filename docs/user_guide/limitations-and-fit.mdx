...
| GQL | `graph.gql()` supports the documented read subset, including node-only `MATCH`, single-node mapped `CREATE`, single-node mapped property `SET`, and single-edge mapped `DELETE`; aggregates, path functions, multi-stage queries, and broader writes are future work |
