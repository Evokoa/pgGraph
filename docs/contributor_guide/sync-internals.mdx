...
when that range is known. It then publishes durable no-overwrite segment files,
validates segment reloads, and publishes the next manifest under an
artifact-root ingestion publication lock. Aborted transaction rows are ignored,
and the manifest watermark advances only after segment publication succeeds.
