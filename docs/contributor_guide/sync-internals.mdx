...
rows. `projection::gc` scans valid manifest metadata, treats the newest
`graph.projection_retention_generations` generations and any generation with an
active heartbeat as protected, and deletes only files that a manifest lists in
`obsolete_files`. The table is extension-owned operational state; PostgreSQL
...
backend-locally, and `graph.active_generation_count()` exposes the active
generation heartbeat count so operators can see whether concurrent backends are
retaining projection generations.
