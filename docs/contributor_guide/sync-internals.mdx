...
workers run build or maintenance jobs. Hosted schedulers can run due durable
sync policy jobs through `graph.run_due_jobs()`, but pgGraph does not broadcast
in-place updates to every backend-local engine.
