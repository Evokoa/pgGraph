...
| Persistence | Atomic `.pggraph` artifacts under PostgreSQL data storage. Format v5 maps nodes, both CSR directions and relationship IDs, resolution, filters and lexical dictionaries, and relationship identities from one validated read-only backend snapshot. |
| Safety | ACL preflight, typed error mapping, selected traversal circuit breakers, mapped-layout validation, and build-memory preflight exist. Graph errors use standard PostgreSQL SQLSTATEs and unwind through pgrx while retaining stable diagnostics. Mapped-layout regressions pass on PostgreSQL 14 through 18 and process-level Valgrind evidence is green; hard byte budgets and other supported-major safety evidence remain in progress. |
...
|---|---|
| Correctness and security | Close remaining coordinate-only RLS visibility, write-identity, isolation, and cross-backend publication gaps before broadening syntax. Parallel-relationship compaction, same-named filter identity, and savepoint overlays are implemented. |
...
| Storage and sync | Format v5 persists both CSR directions and stable relationship/filter identities; durable manifests publish generations with cross-backend compare-and-swap and source watermarks. |
...
| Track | Status | Notes |
|---|---|---|
| Data correctness | Critical baseline landed; P0 follow-ups open | Edge endpoint, exact numeric hydration, parallel relationship compaction, same-named filter identity, and savepoint overlay fixes landed. Broader write identity and relationship-row visibility remain before a full property-multigraph claim. |
...
| SQL boundary safety | PostgreSQL 17 hardening complete; supported-major evidence pending | Source search binds user values through SPI parameters, errors unwind through pgrx before PostgreSQL handles them, definer functions place `pg_catalog` before explicit `pg_temp`, authorization-critical catalog/operator lookups are qualified, and registered relations use OID identity. PostgreSQL 14-16 and 18 release evidence remains. |
...
| Out-of-core build and maintenance | Persisted build and vacuum scan bounded PostgreSQL batches into governed external runs, merge with fixed fanout, stream artifact v6 directly, validate the unpublished candidate, and publish it through generation compare-and-swap. The owned builder remains available for nonpersisted backend-local builds. | Extend production-shaped resource and performance coverage as larger public datasets and additional storage operators are introduced. |
...
| Non-CSR mutable projection | Durable manifest generations, immutable delta CSR segments, layered neighbor reads, trigger-sync relationship identity, parallel-aware compaction and dirty-range replacement, generation-aware garbage collection, repair, diagnostics, and committed-edge cross-backend visibility are implemented for `mutable_overlay`. Maintenance/vacuum is still used to compact accumulated projection state back into a clean base graph. | Finish production verification, release documentation, and benchmark signoff before treating durable projection replacement as fully production-ready. |
