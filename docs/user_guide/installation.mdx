...
The source tree currently represents the pgGraph 1.0.0 release candidate. It is
not yet tagged or published. Build from the reviewed source commit until the
signed PGXN, GHCR, and Homebrew artifacts are announced.
</Callout>
...
release bundle. For the candidate quickstart, build the repository's Compose
image locally.
