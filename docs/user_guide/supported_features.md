...
| Resumable targeted BFS RLS | Implemented after 1.1 | Positive-depth `traverse`, multi-seed BFS, and `get_neighbors` on clean CSR, committed in-memory `edge_buffer` overlays, bounded transaction-local edge overlays, and segment-backed durable projections in `out`, `in`, or `any` direction materialize bounded adjacency candidates, release projection borrows, resolve node and relationship policies in set-based PostgreSQL probes, then admit candidates in original order. The segment-backed `any` cursor preserves the shared Out/In precedence map without materializing whole adjacency. No-RLS/BYPASS execution retains the eager fast path. Transaction-local node/filter changes, DFS, paths, and derived workflows retain the eager oracle until their remaining P4 cursors land. |
