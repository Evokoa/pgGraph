...
| Non-CSR mutable projection | Not current behavior. Current mutable behavior still uses immutable base CSR plus overlays, so maintenance/vacuum is needed to compact committed topology changes. | Investigate an opt-in projection mode built around online-mutable adjacency structures, such as indexed adjacency maps, segmented adjacency blocks, or LSM-like adjacency runs. The goal would be to reduce or remove routine compaction for write-heavy graph workloads while preserving bounded traversal, PostgreSQL source-of-truth semantics, crash safety, and predictable memory behavior. |
