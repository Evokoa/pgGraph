...
`graph.query_freshness = 'apply_pending_sync'` opts wired topology reads into the
same high-water replay primitive before they execute. The wired topology-read
entrypoints are traversal variants, shortest-path queries, weighted shortest-path
queries, component APIs, and `graph.traverse_search()` before its traversal
phase. `graph.search()` remains separate because it reads source-table
properties through SQL rather than graph topology.
