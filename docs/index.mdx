...
<ProjectBadges>
  ...
  <Badge label="version" value="1.0.0 candidate" color="orange" href="https://github.com/Evokoa/pggraph/blob/main/graph/Cargo.toml" />
...
<Callout type="warning">
  The published line remains v0.1.8 alpha. This source contains the pgGraph
  1.0.0 release candidate; it is not yet tagged or published. Its SQL/GQL,
  resource, persistence, migration, and package contracts are frozen for final
  release validation. Build the candidate from source until signed artifacts
  are announced.
</Callout>
...
Persisted graphs are published as immutable generations. A small checksummed
current pointer selects one validated manifest and base artifact, so backends
load either the previous complete generation or the new complete generation,
never a partially written replacement. The graph index remains rebuildable
derived state; PostgreSQL tables remain authoritative.
