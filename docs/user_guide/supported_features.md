...
| Open-vocabulary relationship types | Rebuilt read-only bases use adaptive v7 relationship type storage. Trigger-backed committed `graph.apply_sync()` can deterministically intern valid unseen labels and atomically publish their cumulative dictionary with adaptive segments. Transaction-local/savepoint labels remain assigned to P8.3, and P9 owns the full high-cardinality query and release matrix. See the relationship type limits. |
