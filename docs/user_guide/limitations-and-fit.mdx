...
| GQL | `graph.gql()` supports the documented read subset, including node-only `MATCH`, top-level single-relationship `OPTIONAL MATCH`, projection-stage `WITH`, bounded `DISTINCT`, `RETURN` aggregates, bounded relationship path values and path functions, single-node mapped `CREATE`, single-node mapped property `SET`, and single-edge mapped `DELETE`; SQL/PGQ, full path variable syntax, and broader writes are future work |
