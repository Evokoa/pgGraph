...
expect_value_as_login "$ROLE_NAME" "5" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.gql('MATCH (u:graph_boundary_nodes) RETURN u', hydrate := false);"
expect_value_as_login "$ROLE_NAME" "0" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.gql('MATCH (u:graph_boundary_nodes {id: ''b''}) RETURN u', hydrate := false);"
expect_value_as_login "$ROLE_NAME" "1" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.gql('MATCH (u:graph_boundary_nodes {id: ''a''})-[:boundary_row]->(v:graph_boundary_nodes) RETURN v', hydrate := false);"
expect_value_as_login "$ROLE_NAME" "d" "SET graph.boundary_tenant = 't1'; SELECT string_agg(row #>> '{v,_id,id}', ',' ORDER BY row #>> '{v,_id,id}') FROM graph.gql('MATCH (u:graph_boundary_nodes {id: ''a''})-[:boundary_row]->(v:graph_boundary_nodes) RETURN v', hydrate := false);"
expect_value_as_login "$ROLE_NAME" "d" "SET graph.boundary_tenant = 't1'; SELECT string_agg(row #>> '{v,_id,id}', ',' ORDER BY row #>> '{v,_id,id}') FROM graph.cypher('MATCH (u:graph_boundary_nodes {id: ''a''})-[:boundary_row]->(v:graph_boundary_nodes) RETURN v', hydrate := false);"
expect_value_as_login "$ROLE_NAME" "1" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.gql('MATCH (u:graph_boundary_nodes)-[:boundary_row]->(v:graph_boundary_nodes), (v)-[:boundary_row]->(w:graph_boundary_nodes) WHERE u.id = ''a'' RETURN u, v, w', hydrate := false);"
expect_value_as_login "$ROLE_NAME" "1" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.gql('OPTIONAL MATCH (u:graph_boundary_nodes {id: ''b''}) RETURN u', hydrate := false) WHERE row #>> '{u,_id,id}' IS NULL;"
expect_sqlstate_as_login "$ROLE_NAME" "22000" "SET graph.boundary_tenant = 't1'; SELECT * FROM graph.gql('MATCH (u:graph_boundary_nodes {id: ''b''}) SET u.age = 99 RETURN u', hydrate := false);"
expect_sqlstate_as_login "$ROLE_NAME" "22000" "SET graph.boundary_tenant = 't1'; SELECT * FROM graph.gql('MATCH (u:graph_boundary_nodes {id: ''b''}) REMOVE u.name RETURN u', hydrate := false);"
expect_sqlstate_as_login "$ROLE_NAME" "22000" "SELECT * FROM graph.gql('MATCH (u:graph_boundary_identity_nodes {id: ''hidden''}) DETACH DELETE u RETURN u', hydrate := false);"
expect_sqlstate_as_login "$ROLE_NAME" "22000" "SET graph.boundary_tenant = 't1'; SELECT * FROM graph.gql('MATCH (u:graph_boundary_nodes {id: ''a''})-[r:boundary_row]->(v:graph_boundary_nodes {id: ''c''}) DELETE r RETURN r', hydrate := false);"
expect_sqlstate_as_login "$ROLE_NAME" "42501" "SET graph.boundary_tenant = 't1'; SELECT * FROM graph.gql('MERGE (u:graph_boundary_nodes {id: ''b'', tenant_id: ''t2'', name: ''Bob'', age: 20}) RETURN u', hydrate := false);"
expect_sqlstate_as_login "$ROLE_NAME" "22000" "SET graph.boundary_tenant = 't1'; SELECT * FROM graph.gql('MATCH (u:graph_boundary_nodes {id: ''a''}), (v:graph_boundary_nodes {id: ''b''}) CREATE (u)-[r:boundary_row {visible_to: ''$ROLE_NAME'', edge_weight: 3}]->(v) RETURN r', hydrate := false);"
run_sql "SELECT CASE WHEN (SELECT age FROM public.graph_boundary_nodes WHERE id = 'b') = 20 AND (SELECT name FROM public.graph_boundary_nodes WHERE id = 'b') = 'Bob' AND (SELECT count(*) FROM public.graph_boundary_nodes) = 6 AND (SELECT count(*) FROM public.graph_boundary_edges) = 4 THEN 1 ELSE pg_catalog.current_setting('graph.phase8_write_visibility_regression')::int END;"
expect_sqlstate_as_login "$ROLE_NAME" "42501" "SET row_security = off; SET graph.boundary_tenant = 't1'; SELECT * FROM graph.gql('MATCH (u:graph_boundary_nodes) RETURN u', hydrate := false);"
run_sql "ALTER TABLE public.graph_boundary_nodes FORCE ROW LEVEL SECURITY;"
expect_value_as_login "$ROLE_NAME" "5" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.gql('MATCH (u:graph_boundary_nodes) RETURN u', hydrate := false);"
...
expect_value_as_login "$ROLE_NAME" "a,d,c" "SET graph.boundary_tenant = 't1'; SELECT string_agg(node_id, ',' ORDER BY step) FROM graph.shortest_path('public.graph_boundary_nodes'::regclass, 'a', 'public.graph_boundary_nodes'::regclass, 'c', 20, false, ARRAY['boundary_row']::text[]);"
expect_value_as_login "$ROLE_NAME" "0" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.shortest_path('public.graph_boundary_nodes'::regclass, 'a', 'public.graph_boundary_nodes'::regclass, 'c', 20, false, ARRAY['boundary']::text[]);"
...
expect_value_as_login "$ROLE_NAME" "a,d,c" "SET graph.boundary_tenant = 't1'; SELECT string_agg(node_id, ',' ORDER BY step) FROM graph.weighted_shortest_path('public.graph_boundary_nodes'::regclass, 'a', 'public.graph_boundary_nodes'::regclass, 'c', ARRAY['boundary_row']::text[]);"
...
expect_value_as_login "$ROLE_NAME" "2" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.gql('MATCH p=(u:graph_boundary_nodes)-[:boundary_row*1..2]->(v:graph_boundary_nodes) WHERE u.id = ''a'' RETURN p', hydrate := false);"
expect_value_as_login "$ROLE_NAME" "9" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.connected_components();"
expect_value_as_login "$ROLE_NAME" "5,3,3,9" "SET graph.boundary_tenant = 't1'; SELECT concat_ws(',', num_components, largest_component, num_isolated_nodes, total_active_nodes) FROM graph.component_stats();"
expect_value_as_login "$ROLE_NAME" "5,9,1,5" "SET graph.boundary_tenant = 't1'; SELECT concat_ws(',', count(*), sum(component_size), min(rank), max(rank)) FROM graph.components();"
expect_value_as_login "$ROLE_NAME" "3" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.largest_component(hydrate := false);"
expect_value_as_login "$ROLE_NAME" "3" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.isolated_nodes(hydrate := false);"
expect_value_as_login "$ROLE_NAME" "1" "SET graph.boundary_tenant = 't1'; WITH visible AS (SELECT component_id FROM graph.connected_components() WHERE node_id = 'e') SELECT count(*) FROM graph.component((SELECT component_id FROM visible), hydrate := false);"
expect_value_as_login "$ROLE_NAME" "2,t,f" "SET graph.boundary_tenant = 't1'; WITH req AS (SELECT jsonb_build_object('starts', jsonb_build_array(graph.node_ref_string('public.graph_boundary_nodes'::regclass, 'a')), 'direction', 'out', 'min_depth', 0, 'max_depth', 1, 'edge_types', jsonb_build_array('boundary_row'), 'node_tables', jsonb_build_array('public.graph_boundary_nodes')) AS traversal) SELECT concat_ws(',', estimated_paths, exact, capped) FROM graph.path_count_estimate((SELECT traversal FROM req));"
expect_value_as_login "$ROLE_NAME" "3" "SET graph.boundary_tenant = 't1'; WITH req AS (SELECT jsonb_build_object('starts', jsonb_build_array(graph.node_ref_string('public.graph_boundary_nodes'::regclass, 'a')), 'direction', 'out', 'min_depth', 0, 'max_depth', 1, 'edge_types', jsonb_build_array('boundary_row'), 'node_tables', jsonb_build_array('public.graph_boundary_nodes')) AS traversal) SELECT graph.aggregate(traversal, '{\"count\":[{\"table\":\"graph_boundary_nodes\",\"column\":\"id\",\"as\":\"visible_path_nodes\"}]}'::jsonb, scope := 'all_possible_paths')->>'visible_path_nodes' FROM req;"
expect_value_as_login "$ROLE_NAME" "0" "SET graph.boundary_tenant = 't1'; WITH req AS (SELECT jsonb_build_object('starts', jsonb_build_array(graph.node_ref_string('public.graph_boundary_nodes'::regclass, 'b')), 'direction', 'out', 'min_depth', 0, 'max_depth', 1, 'edge_types', jsonb_build_array('boundary'), 'node_tables', jsonb_build_array('public.graph_boundary_nodes')) AS traversal) SELECT graph.aggregate(traversal, '{\"count\":[{\"table\":\"graph_boundary_nodes\",\"column\":\"id\",\"as\":\"hidden_seed_nodes\"}]}'::jsonb, scope := 'all_possible_paths')->>'hidden_seed_nodes' FROM req;"
run_sql "REVOKE SELECT ON public.graph_boundary_edges FROM $ROLE_NAME;"
expect_sqlstate_as_login "$ROLE_NAME" "42501" "SET graph.boundary_tenant = 't1'; SELECT count(*) FROM graph.connected_components();"
expect_sqlstate_as_login "$ROLE_NAME" "42501" "SET graph.boundary_tenant = 't1'; WITH req AS (SELECT jsonb_build_object('starts', jsonb_build_array(graph.node_ref_string('public.graph_boundary_nodes'::regclass, 'a')), 'direction', 'out', 'min_depth', 0, 'max_depth', 1, 'edge_types', jsonb_build_array('boundary_row')) AS traversal) SELECT * FROM graph.path_count_estimate((SELECT traversal FROM req));"
expect_sqlstate_as_login "$ROLE_NAME" "42501" "SET graph.boundary_tenant = 't1'; WITH req AS (SELECT jsonb_build_object('starts', jsonb_build_array(graph.node_ref_string('public.graph_boundary_nodes'::regclass, 'a')), 'direction', 'out', 'min_depth', 0, 'max_depth', 1, 'edge_types', jsonb_build_array('boundary_row')) AS traversal) SELECT graph.aggregate(traversal, '{\"count\":[{\"table\":\"graph_boundary_nodes\",\"column\":\"id\",\"as\":\"nodes\"}]}'::jsonb, scope := 'all_possible_paths') FROM req;"
run_sql "GRANT SELECT ON public.graph_boundary_edges TO $ROLE_NAME;"
run_sql "REVOKE SELECT ON public.graph_boundary_secret_nodes FROM $ROLE_NAME;"
run_sql "REVOKE CREATE ON SCHEMA graph FROM $ROLE_NAME;"
