...
fn gql_join_relationships_fail_closed_when_edge_row_is_not_visible() {
    reset_and_create_fixtures();
    build_friendship_fixture_graph();
    Spi::run("DROP ROLE IF EXISTS graph_gql_join_relationship_rls").expect("drop role failed");
    Spi::run("CREATE ROLE graph_gql_join_relationship_rls").expect("create role failed");
    Spi::run(
        "ALTER TABLE public.graph_test_friendships_pgtest ENABLE ROW LEVEL SECURITY;
         GRANT USAGE ON SCHEMA graph, public TO graph_gql_join_relationship_rls;
         GRANT SELECT ON public.graph_test_users_pgtest TO graph_gql_join_relationship_rls;
         GRANT SELECT ON public.graph_test_friendships_pgtest TO graph_gql_join_relationship_rls;",
    )
    .expect("create join relationship rls fixture failed");
    create_error_sqlstate_helper();
...
#[pg_test]
fn gql_wildcard_relationships_fail_closed_when_edge_row_is_not_visible() {
    reset_and_create_fixtures();
    build_friendship_fixture_graph();
    Spi::run("DROP ROLE IF EXISTS graph_gql_wildcard_relationship_rls").expect("drop role failed");
    Spi::run("CREATE ROLE graph_gql_wildcard_relationship_rls").expect("create role failed");
    Spi::run(
        "ALTER TABLE public.graph_test_friendships_pgtest ENABLE ROW LEVEL SECURITY;
         GRANT USAGE ON SCHEMA graph, public TO graph_gql_wildcard_relationship_rls;
         GRANT SELECT ON public.graph_test_users_pgtest TO graph_gql_wildcard_relationship_rls;
         GRANT SELECT ON public.graph_test_friendships_pgtest TO graph_gql_wildcard_relationship_rls;",
    )
    .expect("create wildcard relationship rls fixture failed");
    create_error_sqlstate_helper();
...
fn gql_wildcard_path_values_and_functions_have_stable_shape() {
        ...
        Spi::connect(|client| {
            let row = client
                .select(
                    "WITH wildcard AS (
                     ...
                     wildcard_predicate AS (
                         SELECT row
                         FROM graph.gql(
                             'MATCH p=()-[:friend]->(e)
                              WHERE e.name = $target
                              RETURN e, length(p) AS len',
                             params := '{\"target\":\"Bob\"}'::jsonb,
                             hydrate := false
                         )
                     ),
                     ...
                     SELECT count(*)::bigint,
                            ...
                            (SELECT bool_and(
                                ...
                                AND row->>'peer' = 'Alice'
                                AND row->>'target' = 'Bob'
                             ) FROM joined_friend_predicate),
                            (SELECT bool_and(
                                row->>'source' = 'Alice'
                                AND row->>'target' = 'Bob'
                             ) FROM joined_friend_unhydrated_property),
                            (SELECT count(*)::bigint FROM wildcard_predicate),
                            (SELECT bool_and(
                                (row->>'len')::bigint = 1
                                AND row->'e' ? '_id'
                                AND NOT (row->'e' ? 'name')
            ...
            Ok::<_, pgrx::spi::Error>((
                ...
                row.get::<bool>(7)
                    ...
                    .unwrap_or(false),
                row.get::<bool>(8)
                    .expect("join predicate read failed")
                    .unwrap_or(false),
                row.get::<bool>(9)
                    .expect("unhydrated join property read failed")
                    .unwrap_or(false),
                row.get::<i64>(10)
                    .expect("wildcard predicate row count read failed")
                    .unwrap_or_default(),
                row.get::<bool>(11)
    ...
    assert!(join_predicate_matches);
    assert!(unhydrated_join_property_matches);
    assert_eq!(
        wildcard_predicate_rows, 1,
    ...
    let join_relationship_shape = Spi::get_one::<bool>(
        "SELECT bool_and(
                    row->'r'->>'_type' = 'friend'
                    AND row->'r'->'_start'->>'table' = 'graph_test_users_pgtest'
                    AND row->'r'->'_start'->>'id' = 'u1'
                    AND row->'r'->'_end'->>'table' = 'graph_test_users_pgtest'
                    AND row->'r'->'_end'->>'id' = 'u2'
                )
         FROM graph.gql(
             'MATCH (u:graph_test_users_pgtest)-[r:friend]->(c:graph_test_users_pgtest),
                    (v:graph_test_users_pgtest)-[:friend]->(c)
              RETURN r',
             hydrate := false
         )",
    )
    .expect("multi-pattern relationship projection query failed")
    .unwrap_or(false);

    assert!(join_relationship_shape);

    let join_path_shape = Spi::get_one::<bool>(
        "SELECT bool_and(
                    row->'path'->'_path'->'nodes'->0->'_id'->>'id' = 'u1'
                    AND row->'path'->'_path'->'nodes'->1->'_id'->>'id' = 'u2'
                    AND row->'path'->'_path'->'relationships'->0->>'_type' = 'friend'
                    AND row->'path'->'_path'->'relationships'->0->'_start'->>'id' = 'u1'
                    AND row->'path'->'_path'->'relationships'->0->'_end'->>'id' = 'u2'
                )
         FROM graph.gql(
    ...
    .expect("multi-pattern path projection query failed")
    .unwrap_or(false);
    ...
    let join_var_len_shape = Spi::get_one::<bool>(
        "SELECT bool_and(
                    row->'ns'->0->'_id'->>'id' = 'u1'
                    AND row->'ns'->1->'_id'->>'id' = 'u2'
                    AND row->'rs'->0->>'_type' = 'friend'
         ...
         FROM graph.gql(
             'MATCH p=(u:graph_test_users_pgtest)-[:friend*1..1]->(c:graph_test_users_pgtest),
                    (v:graph_test_users_pgtest)-[:friend]->(c)
              RETURN nodes(p) AS ns, relationships(p) AS rs, length(p) AS len',
             hydrate := false
         )",
    )
    .expect("multi-pattern variable-length projection query failed")
    .unwrap_or(false);
    ...
    .expect("multi-pattern aggregate WITH projection query failed")
    .unwrap_or(false);
