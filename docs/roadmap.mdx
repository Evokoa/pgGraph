...
| PostgreSQL 19 | Support native `CREATE PROPERTY GRAPH` definitions as an authoritative mapping source without duplicate pgGraph registration; add version-gated catalog, DDL invalidation, security, SQL/PGQ differential, packaging, and CI support while retaining PostgreSQL 14-18 frontends. |
| Refactoring and Rust boundaries | Decompose build, projection, binder, executor, evaluator, engine, persistence, SQL facade, sync, and tests while replacing stringly closed state with canonical enums, raw cross-domain integers with production newtypes, JSON/`f64` semantic values with exact graph values, and scattered unsafe/PostgreSQL calls with pgrx-first reviewed adapters. |
