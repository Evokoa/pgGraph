...
- `graph.reset()` remains projection-only. `graph.reset(true)` additionally
  clears the selected graph's table, edge, and filter registrations while
  preserving PostgreSQL source rows and other named graphs.
