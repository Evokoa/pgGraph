...
The unreleased 1.1 single-seed BFS form of `graph.traverse()` builds one
statement-scoped visibility scope after freshness handling. For each registered
source relation where PostgreSQL says RLS applies to the outer caller, pgGraph
scans caller-visible source keys and intersects them with projected node and
relationship identities. Hidden seeds behave as nonexistent, and hidden nodes
or relationship rows are rejected before they affect reachability, paths,
limits, or hydration.
...
branch. The current vertical slice covers only single-seed BFS
`graph.traverse()`. Multi-seed traversal, DFS, path, component, aggregation,
GQL, Cypher, and workflow paths remain builder-scoped until their ordered 1.1
phases land. Do not treat those APIs as tenant-isolation boundaries yet.
</Callout>
