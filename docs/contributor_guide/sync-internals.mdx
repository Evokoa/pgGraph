...
Projection segment versions are not read across this format boundary. After an
upgrade from a release that wrote segment format v2, rebuild the graph from its
authoritative PostgreSQL tables before serving the mutable projection. A clean
