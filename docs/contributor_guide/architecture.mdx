...
tagged with the graph id/name that produced the active engine, so runtime calls
can reject or clear a stale engine before serving a different selected graph.
`graph.select_graph()` changes the session selection, `graph.load_graph()`
loads one persisted artifact into the backend-local slot, and
`graph.loaded_graphs()` reports zero or one row for the current backend.
