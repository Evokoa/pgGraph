...
| `edge_type_registry` after load | `Engine` heap | Bounded labels are decoded from their v5 section |
...
| Persistence | The graph artifact is stored as `$PGDATA/<graph.data_dir>/<graph_id>/main.pggraph`. |
...
<PipelineDiagram>
  ...
  <PipelineStep title="validate v5 header, both CRCs, 23 descriptors, CSR, PK offsets, filters, and identities">
          ...
          <TreeNode title="graph.maintenance()">
            <TreeNode title="fresh build from source tables">
              <TreeBranch>
                ...
                <TreeNode title="base-only projection generation" />
              </TreeBranch>
            </TreeNode>
          </TreeNode>
        </TreeBranch>
