...
| Resumable targeted BFS RLS | Implemented after 1.1 | Positive-depth `traverse`, multi-seed BFS, and `get_neighbors` on clean CSR materialize bounded adjacency candidates, release projection borrows, resolve node and relationship policies in set-based PostgreSQL probes, then admit candidates in original order. No-RLS/BYPASS execution retains the eager fast path. Committed overlays, segment-backed durable projections, transaction-local topology, DFS, paths, and derived workflows retain the eager oracle until P4 adds representation-specific cursors. |
