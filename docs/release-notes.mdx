...
18. The source tree is release-ready but unpublished. PostgreSQL source tables
remain authoritative, while graph artifacts and mutable projection generations
remain validated, rebuildable derived state.
...
Release readiness requires retained RC/full-matrix evidence, clean PostgreSQL
...
consume those exact prepared bytes after signed-tag verification; it must not
rebuild the archive.
