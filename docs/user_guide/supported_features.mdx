...
The relationship-identity completeness summary covers the base CSR, durable
segments, committed in-memory deltas and transaction-local state. Query startup
checks the relevant type summaries instead of scanning every relationship
identity. A relevant identity gap fails closed with `PG023`; legacy base gaps
require a rebuild even when a later tombstone masks the affected relationship.
Summary checks still perform work over relevant types and bounded mutable state.

The deterministic targeted-lazy/global-eager selector uses the supported query
shape and current projection state. Global queries and unsupported resumable
shapes retain eager visibility preparation. Eager relationship preparation uses
a temporary, governed lookup per active mapping; its cost still grows with the
relationship dictionary and visible source set.

The 1.2.1 candidate has retained
[46 exact ordered comparisons and two backend cleanup checks](../../release/evidence/engine/2026-09-11-selective-rls-exact/README.md)
and complete [1M](../../release/evidence/engine/2026-09-11-p5-selective-rls-1m/README.md)
and [10M scale profiles](../../release/evidence/engine/2026-09-12-p5-selective-rls-10m/README.md).
Each scale contains that many scalar nodes plus that many composite nodes.
Both automatic identity queries returned the same row as forced eager execution
while returning three policy-source rows, versus three million and thirty
million respectively. The 10M run reached its 6 GiB container memory ceiling
without an OOM on a 12 GiB outer VM. These four-case profiles do not qualify
every topology query shape or establish a latency SLA.

Visibility `source_rows` counts rows returned by policy probes or scans, not
all PostgreSQL rows examined or hydration work. Enforced targeted one-hop GQL
can skip repeated read rechecks, while eager/global and no-RLS routes retain
them. Zero read rechecks therefore does not mean zero additional source SQL.
...
- new PostgreSQL major compatibility and native SQL/PGQ integration if it returns upstream;
