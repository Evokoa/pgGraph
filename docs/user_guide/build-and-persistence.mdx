...
reads. Rebuild and vacuum preflight uses a conservative peak-build estimate
against the currently loaded engine, so large rebuilds fail before the backend
allocates the replacement graph. If the estimate exceeds
`graph.memory_limit_mb`:

| `graph.oom_action` | Behavior |
|---|---|
| `error` | Raise SQLSTATE `53200` with diagnostic `PG001` and stop before allocating the engine |
