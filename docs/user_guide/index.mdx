...
not part of the PostgreSQL 14–18 contract.
</Callout>
...
registered graph definition
      ...
      | graph.build()
      v
validated derived projection
      |
      | graph.persist_on_build = true
      v
generation artifacts under PostgreSQL-managed storage
      |
...
Within that graph-specific artifact root, `main.pggraph` is the logical
compatibility path. Current persisted builds publish immutable generation base
files and select one through `projection-current.json` plus its checksummed
manifest. Loaders follow that selected generation, while stores without
projection metadata can still use the compatibility path.

For day-to-day SQL use, that is the whole model: authoritative source tables,
a registered graph definition, a rebuildable derived projection, and ordinary
SQL results. Implementation details are documented in the
