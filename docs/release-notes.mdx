...
The candidate is validated through retained RC/full-matrix evidence, clean
...
and playground examples. The source candidate bundle contains one commit-bound
PGXN ZIP, SPDX 2.3 SBOM, provenance, manifest, and complete SHA-256 set.
Publication consumes those exact prepared bytes after signed-tag verification;
it does not rebuild the archive.
