...
| Reset | `graph.reset()` clears the backend-local engine and removes files for the selected graph's artifact root while preserving registrations; `graph.reset(true)` also clears that selected graph's registrations. |
