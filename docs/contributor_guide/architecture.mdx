...
`graph.status()`, `graph.traverse()`, and both `graph.reset()` forms resolve the
selected graph, or the default graph when no graph has been selected. The
default graph behavior remains available while named graph identity is explicit
in catalogs, jobs, artifacts, runtime loading, sync replay, and projection
metadata.
...
| `edge_type_registry` after load | `Engine` heap | Bounded labels are decoded from the v6 inline section or the v7 cumulative dictionary |
...
| Persistence | Each graph uses `$PGDATA/<graph.data_dir>/database-<database_oid>/<graph_id>/` as its artifact root. `main.pggraph` is the logical compatibility path; current persisted builds serve the generation-specific base selected by `projection-current.json` and its manifest. |
...
| Reset | `graph.reset()` clears the backend-local engine and transactionally removes the selected graph's publication while preserving registrations and rollback-visible files; `graph.reset(true)` also clears that selected graph's registrations. |
