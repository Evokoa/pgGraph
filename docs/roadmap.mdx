...
| Safety | ACL preflight, typed error mapping, selected traversal circuit breakers, and build-memory preflight exist. Hard byte budgets are not universal, mapped safe APIs need stronger validated-layout types, and custom SQLSTATE emission must move back behind pgrx's guarded PostgreSQL error boundary. |
...
| SQL boundary safety | Search hardened; P0 boundary follow-ups open | Source search binds user values through SPI parameters. Coordinate-only GQL topology needs a fail-closed visibility gate; security-definer search paths, OID-stable relation identity, and custom SQLSTATE guard behavior also require explicit fixes. |
