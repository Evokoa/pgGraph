...
and a complete [1M scale profile](../../release/evidence/engine/2026-09-11-p5-selective-rls-1m/README.md).
The latter contains one million scalar and one million composite nodes. Its
automatic identity query returned the same row as forced eager execution while
returning three policy-source rows rather than three million. The separate
10M qualification is not complete.
