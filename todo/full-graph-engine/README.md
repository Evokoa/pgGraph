...
| Equal endpoint/type relationships still lack identity in every non-base path. | `graph/src/projection/layered.rs`, `graph/src/projection/neighbors.rs`, `graph/src/sql_facade/gql.rs` | Finish edge-row identity through overlays, sync, compaction, writes, and relationship-row RLS; keep base CSR GQL hydration keyed by source-row identity. |
