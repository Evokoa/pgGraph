...
mmap-backed fixed-width arrays after persistence. PostgreSQL backends do not
share Rust heap memory. After auto-load, they share the fixed-width persisted
base graph pages through the operating system page cache, while derived and
variable-size structures remain per-backend heap allocations.
