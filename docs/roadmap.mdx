...
| Querying | Bounded traversal, shortest paths, weighted shortest paths, path reconstruction, source search, workflow helpers, connected components, and a read-only GQL-compatible subset through `graph.gql()`. |
...
| Broader GQL and SQL/PGQ frontends | A read-only GQL-compatible subset is public through `graph.gql()`; SQL/PGQ, GQL writes, and broader GQL coverage are not current public APIs. | Extend GQL coverage and investigate a SQL/PGQ adapter path over the shared graph planner only when compatibility rows are green. |
| Online mutable graph overlays | The `mutable_overlay` build mode and transaction-delta lifecycle are present, but public graph write clauses are not current behavior. | Route narrow GQL writes through PostgreSQL-first DML and transaction-local deltas while keeping `csr_readonly` as the fast read-mostly mode. |
