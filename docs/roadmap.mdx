...
| Broader GQL and SQL/PGQ frontends | A GQL-compatible subset, including node-only `MATCH`, single-relationship reads, top-level single-relationship `OPTIONAL MATCH`, projection-stage `WITH`, bounded `DISTINCT`, `RETURN` aggregates, bounded relationship path values and path functions, single-node mapped `CREATE`, single-node mapped property `SET`, and single-edge mapped `DELETE` are public through `graph.gql()`; SQL/PGQ and broader GQL coverage are not current public APIs. | Extend GQL coverage and investigate a SQL/PGQ adapter path over the shared graph planner only when compatibility rows are green. |
| Online mutable graph overlays | The `mutable_overlay` build mode, transaction-delta lifecycle, overlay-aware read paths, PostgreSQL-first GQL node `CREATE`, PostgreSQL-first GQL property `SET`, and PostgreSQL-first GQL edge-row `DELETE` are present. | Continue routing narrow GQL writes through PostgreSQL-first DML and transaction-local deltas while keeping `csr_readonly` as the fast read-mostly mode. |
