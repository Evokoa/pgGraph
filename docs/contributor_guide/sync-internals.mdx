...
same high-water replay primitive before they execute. The current rollout wires
single-start `graph.traverse()` first; other topology read entrypoints remain on
the compatibility path until the broader rollout stage.
