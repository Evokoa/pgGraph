...
Enforcement can be `hard` or `warn`. The current hard enforcement point is
`max_named_graphs`: pgGraph checks cluster and owner limits before creating a
graph metadata row. Warning quotas are visible through
