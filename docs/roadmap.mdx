...
| Broader GQL and SQL/PGQ frontends | A GQL-compatible subset, including node-only `MATCH`, single-relationship reads, top-level single-relationship `OPTIONAL MATCH`, projection-stage `WITH`, bounded `DISTINCT`, `RETURN` aggregates, JSONB list/map property paths, bounded relationship path values and path functions, single-node mapped `CREATE`, single-node mapped property `SET`, mapped property `REMOVE`, single-edge mapped `DELETE`, and single-node `DETACH DELETE` over registered edge-row relationships are public through `graph.gql()`; an internal typed SQL/PGQ adapter seam exists, but SQL/PGQ and broader GQL coverage are not current public APIs. | Extend GQL coverage and expose SQL/PGQ only after PostgreSQL provides stable graph-pattern hooks and the compatibility rows are green. |
| Online mutable graph overlays | The `mutable_overlay` build mode, transaction-delta lifecycle, overlay-aware read paths, PostgreSQL-first GQL node `CREATE`, PostgreSQL-first GQL property `SET`, PostgreSQL-first GQL property `REMOVE`, PostgreSQL-first GQL edge-row `DELETE`, and PostgreSQL-first GQL node `DETACH DELETE` are present. | Continue routing narrow GQL writes through PostgreSQL-first DML and transaction-local deltas while keeping `csr_readonly` as the fast read-mostly mode. |
