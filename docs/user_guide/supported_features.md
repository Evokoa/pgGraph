...
| Resumable targeted BFS RLS | Implemented after 1.1 | Positive-depth `traverse`, multi-seed BFS, and `get_neighbors` on clean CSR, committed in-memory `edge_buffer` overlays, and bounded transaction-local edge overlays materialize bounded adjacency candidates, release projection borrows, resolve node and relationship policies in set-based PostgreSQL probes, then admit candidates in original order. No-RLS/BYPASS execution retains the eager fast path. Segment-backed durable projections, transaction-local node/filter changes, DFS, paths, and derived workflows retain the eager oracle until their P4 cursors land. |
