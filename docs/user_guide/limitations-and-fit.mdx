...
| SQL-first applications | The current public API is PostgreSQL SQL functions in the `graph` schema, including the `graph.gql()` SQL function for the documented read-only GQL subset |
...
| GQL | `graph.gql()` supports the documented read-only Phase 1 subset; writes, aggregates, path functions, and multi-stage queries are future work |
| SQL/PGQ | Planned adapter work, not a current public API |
| Projection modes | `graph.build()` defaults to `csr_readonly`; `mutable_overlay` can be selected when `graph.mutable_enabled = on`, but public graph write clauses are still future work |
