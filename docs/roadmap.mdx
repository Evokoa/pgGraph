...
| Out-of-core build and maintenance | Low-memory rebuild mode reduces old/new overlap, governed external runs and multi-pass merge are available, and artifact v5 maps inbound/filter/identity sections. Directly streaming complete builds from runs into the publication candidate remains open. | Connect build runs to direct validated artifact publication, retain bounded range compaction, and keep deterministic resource SQLSTATEs. |
