...
or another host-managed worker to call `graph.run_job(job_id)` or
`graph.run_sync_policy(policy_id)` at the cadence recorded on the policy.
...
jobs produce a `disabled` run row and do not apply sync.
