...
| Online mutable graph overlays | The `mutable_overlay` build mode, transaction-delta lifecycle, and overlay-aware read paths are present for traversal, unweighted shortest path, connected components, and read-only GQL pattern expansion. Public graph write clauses are not current behavior. | Route narrow GQL writes through PostgreSQL-first DML and transaction-local deltas while keeping `csr_readonly` as the fast read-mostly mode. |
